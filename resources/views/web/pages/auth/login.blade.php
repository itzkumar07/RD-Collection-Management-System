...
@section('auth-section')
    <figure>
        <form action="{{ route('web.view.login') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                <p class="text-xs">Forgot your password ? <a href="{{route('web.view.forgot.password')}}" class="link">Reset Password</a></p>
            </div>

            <div>
                <p class="text-xs">Create a new account ? <a href="{{route('web.view.register')}}" class="link">Register now</a></p>
            </div>
